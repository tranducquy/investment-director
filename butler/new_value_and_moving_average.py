...
class Butler():
    ...
    def check_open_long(self, q, idx):
        ...
        #past_close = self._get_maximum_close_price_for_nv(q, idx)
    ...
    def check_open_short(self, q, idx):
        ...
        #past_close = self._get_minimum_close_price_for_nv(q, idx)
        #today_open = q.quotes['open'][idx]
    ...
    def create_close_order_stop_market_long(self, q, idx):
        if q.quotes['low'][idx] is None:
            return -1
        price = q.quotes['low'][idx] - self.tick
        return price
    ...
    def _get_maximum_close_price_for_nv(self, q, idx):
        maxlength = len(q.quotes)
        if maxlength < self.nv_duration or idx < self.nv_duration:
            return -1
    ...
    def _get_minimum_close_price_for_nv(self, q, idx):
        maxlength = len(q.quotes)
        if maxlength < self.nv_duration or idx < self.nv_duration:
            return -1
        ary = q.quotes['close'][idx-self.nv_duration:idx]
